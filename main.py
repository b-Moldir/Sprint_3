...
class IncorrectPassword:
    def test_password(self, invalid_credentials, driver=None):
...
#Ожидание сообщение об ошибке
        WebDriverWait(driver,5).until(expected_conditions.visibility_of_element_located(*Locators.TEXT_INCORRECT_PASSWORD))
#Проверка на неверный пароль
        text_incorrect_password = driver.find_element(*Locators.TEXT_INCORRECT_PASSWORD).text
        assert "Такой пользователь уже существует" in text_incorrect_password
